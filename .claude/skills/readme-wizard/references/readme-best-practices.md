...
- [Repos Without a Git Remote](#repos-without-a-git-remote)
- [Vocabulary Rules in This Repo](#vocabulary-rules-in-this-repo)
...
**Specification/planning repos (no code yet):**
- Lead with what the project is and what decisions have already been made
- The document map replaces the Quick Start — a Quick Start for software that does not exist is a lie
- Say plainly which phase the project is in; a reader arriving expecting a runnable system deserves to learn otherwise in the first paragraph
- Link the source-of-truth documents in the documentation table and note which are superseded
...
A directory can be a real project without being a GitHub repository. When `scan-project.ts` reports `is_git_repo: false`, or reports it as a repo with no origin remote, everything that resolves through `github.com/<owner>/<repo>` is unavailable:

- No shields.io GitHub badges — license, version, build, and stars all read the GitHub API
- No `contrib.rocks` contributor avatars
- No star-history chart
- No "Fork this repo" or clone instructions pointing at a URL you do not have

This is not a degraded README. A hero with a clear name and one accurate sentence, followed by a document map, reads better than a hero padded with badges that render as broken images. Add those sections when the remote exists, not before.

The scan's `notes` array names each of these omissions explicitly. `check-readme.ts` fails the README if one appears anyway.

## Vocabulary Rules in This Repo

`.claude/rules/design-standards.md` bans two registers from user-facing copy, and the README is user-facing copy:

- **Marketing language** — "powerful", "seamless", "blazing fast", "world-class", "revolutionary". It asks the reader to be impressed instead of informed. `check-readme.ts` fails on a list of these.
- **Implementation language** — internal service names, class names, and architecture vocabulary used before it has been introduced. It asks the reader to understand the system before they understand what it is for.

The operator-facing parts of a README may use the project's real domain vocabulary; the opening paragraphs may not.

## Common Pitfalls

- **Placeholder text left in.** Things like `{{PROJECT_NAME}}`, `TODO`, `Lorem ipsum`. `scripts/check-readme.ts` catches these, but it's worth a manual skim too.
...
- **Outdated install commands.** If the scan detects pnpm, use `pnpm install`, not `npm install`. Match the project's actual package manager. In this repo that means `bun`, and `uv` for Python tooling.
