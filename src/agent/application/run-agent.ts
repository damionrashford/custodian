import { canonicalJson, err, ok, type Result } from "@custodian/primitives";
import { evaluateLoop } from "../domain/loop-controls";
import { resolvePromptVersion, type PromptSnapshot } from "@custodian/governance";
import { serveCompletion, type CompletionResponse } from "@custodian/serving";
import { parseRequestHash } from "@custodian/serving";
import type { ToolSummary } from "../domain/tool-catalogue";
...
export async function runAgent(
  ...
  const snapshot = resolvePromptVersion(deps.registry, request.deployment);
  if (!snapshot.ok) {
