...
import { parseModelSnapshot, parsePromptVersion } from "@custodian/primitives";
...
test("a label resolves to the snapshot it points at", () => {
  const resolved = resolvePromptVersion(twoVersionsLive(), "production");
...
test("resolving an unset label is an error, not a silent default", () => {
  expect(resolvePromptVersion(twoVersionsLive(), "canary")).toEqual({
    ok: false,
...
test("a snapshot records everything needed to roll back without guessing", () => {
  const resolved = resolvePromptVersion(twoVersionsLive(), "production");
  if (!resolved.ok) throw new Error("resolvePromptVersion failed");
...
test("canary and production are independent labels over the same history", () => {
  ...
  const canary = resolvePromptVersion(withCanary.value, "canary");
  const production = resolvePromptVersion(withCanary.value, "production");
  if (!canary.ok || !production.ok) throw new Error("resolvePromptVersion failed");
