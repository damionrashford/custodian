...
Before picking, check the target codebase for a `/* Studio · macrostructure: <name> · ... */` comment in any existing CSS file. If you find one, **your pick must be a different macrostructure.** The pick must differ from the last 3 entries in `.studio/log.json` (matches SKILL.md STEP 2); at minimum, no two consecutive Studio outputs in the same project share a macrostructure.
...
Each macrostructure also implies a **nav archetype** (N1–N10) and a **footer archetype** (Ft1–Ft8). The defaults sit in the routing tables in [`component-cookbook.md`](component-cookbook.md) § Navigation and § Footers. Don't ship a hero macrostructure without picking nav + footer alongside — they are part of the page shape, not optional chrome.

---

## The 21 macrostructures — index

**Pick one. Then read ONLY that one file** from `macrostructures/`. Do not load the whole catalogue. Slugs are stable; the diversification rule reads the `<name>` from the `/* Studio · macrostructure: <name> · ... */` stamp.
...
2. **Social proof / logo wall** — 6–8 customer logos in monochrome. (See [`assets.md` § Logo walls](../rules/assets.md).)
...
4. **Testimonials** — 2–4 quote-card SLOTS. Pull-quote + name + role + company, photo optional. Only fill them with quotes the user supplied; for placeholder briefs, ship labelled grey blocks with `—` placeholders and a "quotes to confirm" note (SKILL.md invariant: no fabricated content; gate 56). When real quotes exist, prefer use-case-specific ones over "We use [product] every day" filler.
...
- **Testimonials:** include the quoted person's role *and* company when the user supplied the quote. Abstract "Engineering Manager" testimonials are slop. NEVER invent names or quotes — placeholder briefs get labelled `—` slots, not plausible-sounding people (no-fabrication invariant + gate 56; "Jane Doe" / "John Smith" additionally trips gate 20).
...
2. **Check the codebase** for an existing `/* Studio · macrostructure: <name> · ... */` stamp. If found, exclude that name from your choices.
...
The goal is not novelty for its own sake. The goal is that two pages Studio builds for two different briefs *look like different sites, not different colour-swaps of the same template*.
