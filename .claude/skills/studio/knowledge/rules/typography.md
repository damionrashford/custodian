...
| **The Future** | (in repo) | Studio's own body workhorse | Default Studio tone |
...
| Family | Source | Voice | Best for |
| --- | --- | --- | --- |
| **Geist Mono** | Google | Geist's mono companion | Default Studio mono, code, captions |
...
| **Workshop** *(Studio's own theme)* | Free | The Future · Geist · Cabinet Grotesk | The Future · Switzer | The Future Mono · Geist Mono |
...
Pick a ratio. The default for Studio work is **1.25** (major third); alternatives by register: 1.333 perfect fourth (classic default, roomier) · 1.414 augmented (editorial) · 1.5 perfect fifth (display-heavy) · 1.618 golden (luxury). Every size on the page derives from the chosen ratio — don't mix scales. Build from a 16px body, then clamp display sizes for responsive.
...
Use no more than **five text sizes on a page, and no more than three within any one section or component**. If you need more hierarchy, use weight and colour, not another size. (The critique checklist and slop gates enforce the same numbers.)
...
- Canonical pairs by register: **300/600** (luxury / refined) · **400/700** (default) · **500/800** (display-heavy). Anti-pattern: 400/500 — too close, reads as a rendering bug.
- Weight contrast beats size contrast: 24px Bold + 24px Regular is clearer hierarchy than 28px Regular + 24px Regular. Reach for weight first.
- Never synthesise. Load the weight you need; don't rely on `font-weight: bold` against a single-weight file.
  Cite: Wathan & Schoger, *Refactoring UI* — font weight hierarchy.
...
Line-height is set per role, never globally:

```
Body             1.5–1.625
Long-form prose  1.6–1.75 (use serif)
UI labels        1.2–1.3 (12–14px)
Buttons          1.0 (single line, centered via padding)
Headlines        1.05–1.15 (tighter as size grows)
Display          1.0–1.05 (all-caps floor is 1.0 — see below)
```

Cite: Butterick, *Practical Typography* — line spacing; Lupton, *Thinking with Type*.
...
- Hang punctuation at display sizes: an opening quote on a display headline hangs into the left margin so the glyphs align with the body below — `hanging-punctuation: first;`. Cite: Bringhurst, *Elements of Typographic Style* — hung punctuation.
- Letter-spacing follows size: display (>40px) −0.02 to −0.04em; body (14–18px) zero; labels (<14px) +0.02 to +0.05em, usually uppercase. Cite: Butterick, *Practical Typography* — letterspacing.
...
- [ ] Display font is NOT Inter / Roboto / Open Sans / Arial / system-ui
- [ ] ≤3 font families total; outlier face in ≤2 slots
- [ ] ≤5 text sizes on the page; ≤3 within any one section
- [ ] Every size traces to the chosen scale ratio
- [ ] Weight pair is 300/600, 400/700, or 500/800 — body/headings contrast ≥300 units
- [ ] Body measure 45–75ch (`max-width: 65ch` default)
- [ ] Body line-height 1.5–1.65; display 1.05–1.2; all-caps display ≥1.0
- [ ] Hero headline size matches copy-length bucket (see Scale § Hero headline sizing)
- [ ] Tabular figures on every numeric column (`font-variant-numeric: tabular-nums`)
- [ ] Curly quotes, real em-dashes (—), real ellipses (…)
- [ ] `font-display: swap` + metric-matched fallbacks on every web font
- [ ] Free-tier faces unless the user confirmed a foundry licence

## Drill-deeper queries

```bash
bun ${CLAUDE_SKILL_DIR}/scripts/library/search.ts --book thinking-with-type "hierarchy"
bun ${CLAUDE_SKILL_DIR}/scripts/library/search.ts --book elements-of-typographic-style "measure"
bun ${CLAUDE_SKILL_DIR}/scripts/library/search.ts --book practical-typography "line spacing"
bun ${CLAUDE_SKILL_DIR}/scripts/library/search.ts --book refactoring-ui "font weight"
bun ${CLAUDE_SKILL_DIR}/scripts/library/search.ts --domain typography "type pairing display body"
```
