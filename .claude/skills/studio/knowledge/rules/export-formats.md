...
Loaded by [`SKILL.md`](../../SKILL.md) STEP 9 when emitting the design system as portable tokens. Defines the four canonical formats Studio always writes:
...
## Token taxonomy — Studio's source of truth

Every Studio output writes these tokens (or a subset, if the page doesn't use one). The names are the source; every other format is a translation.

| Studio token | Type | Example value |
| --- | --- | --- |
...
The source. Plain CSS custom properties at `:root`. Every Studio page CSS imports this file at the top:
...
Tailwind v4 reads CSS variables inside `@theme` and generates utilities (`bg-paper`, `text-ink`, etc.) automatically. The translation from Studio tokens is direct — same variable names, same OKLCH values:
...
@theme {
  ...
  /* Spacing — Tailwind reads --spacing-* by default; we keep Studio's --space-* names alongside */
  --spacing-3xs: 0.25rem;
  --spacing-2xs: 0.5rem;
  --spacing-xs:  0.75rem;
  --spacing-sm:  1rem;
...
Notes:
- Tailwind v4 expects `--spacing-*` (with `ing`) for the spacing utilities. We mirror Studio's `--space-*` to `--spacing-*` so both names work.
...
The translation from Studio tokens:
...
:root {
  ...
  --accent:               58%   0.16  60;     /* accent (same as primary in Studio) */
...
Then the user's `tailwind.config.ts` (or v4 `@theme`) reads them with `oklch(var(--background))`. Studio's accent → shadcn's primary is the canonical mapping; secondary → paper-3.

If the user wants a dark variant, mirror with the dark theme tokens under a `.dark` selector (Studio's Midnight / Bloom / Terminal supply the values).

---

## Output rule

When SKILL.md STEP 9 emits exports:
