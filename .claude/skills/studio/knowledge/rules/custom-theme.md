...
**The freedom is the combination, not the rules.** Every constraint in [`color.md`](color.md), [`typography.md`](typography.md), and [`anti-patterns.md`](anti-patterns.md) still applies. The 71 slop-test gates fire unchanged. The STEP 2 picks statement surfaces the palette + pairing in plain text *before* any code is emitted, so the user can redirect.

## Two words, plain English

- **catalog** — the named-theme catalogue. Studio's 22 themes (Specimen, Atelier, Brutal, Salon, Newsprint, Linen, Studio, Manifesto, Terminal, Midnight, Almanac, Garden, Quiet, Riso, Sport, Bloom, Coral, Violet, Aurora, Halo, Plume, Editorial). Each one is a fixed combination of paper-band, display-style, and accent-hue. The rotation rule cycles through them so two consecutive runs don't read alike. **This is the default.** Most briefs use it.
...
Studio must **not** offer catalog-vs-custom on every prompt. That's friction, not discipline. Surface the fork only when the brief carries one of these signals:
...
Write all three into the macrostructure stamp (§ E below) and the `.studio/log.json` entry (§ F below). They are the durable record. The next run reads them.
...
```css
/* Studio · macrostructure: <name> · <hero archetype + knobs>
...
```css
/* Studio · macrostructure: Long Document · H5 hero knobs: salutation=time-stamp, body=2 paragraphs, signoff=initials
...
Catalog entries continue to record `theme: <name>` and skip `theme_axes` (the catalog's axes are looked up from [`themes.md`](themes.md)). Step 2.5 logic uses the same diversification check on both — for catalog entries it reads the axes from tokens.css; for custom entries it reads them from the entry.
...
**Stamp:**
```css
/* Studio · macrostructure: Workbench · F2 sticky-scroll knobs: pinned=right, content=trace-panel, steps=3
...
**Stamp:**
```css
/* Studio · macrostructure: Catalogue · F1 catalogue knobs: tiles=8, columns=2, rule=hairline-between
...
2. **Does not save themes for reuse.** A custom run is per-output. The skill does not write back to the theme catalog in [`themes.md`](themes.md). If the user wants a permanent theme, they paste the custom palette into tokens.css themselves and name it.
...
5. **Does not bypass the STEP 2 picks statement.** The custom palette + pairing surface in plain text *before* any code is emitted, so the user can redirect early.
