# Slop test — 71 gates + pre-emit self-critique

Run this list before handing back any output. Every answer must be **no**. Update the STEP 9 quality block's `Slop gates` row to reflect the actual outcome of this run.
...
Run this **before** the gate list, not after. Score the planned output 1–5 on each axis. Anything **< 3 on any axis triggers a revision pass** before the gate sweep — don't bring known weakness into a seventy-one-gate review.
...
| **F** | **Variety** | Does this output share a structural fingerprint with a previous Studio output in the project? Score by structural distance, not visual distance — colour-swaps don't count as variety. |

Record the six scores in a one-line stamp comment at the top of the file: `/* Studio · pre-emit critique: P5 H4 E5 S4 R5 V5 */`. Future runs should be able to find this and avoid repeating the same weakness.
...
21. Is the `/* Studio · macrostructure: <name> · ... */` stamp missing from the top of the CSS? (It must be present.)
22. Is the macrostructure I picked the same as a previous Studio output's stamp in this project? (Read the file system; if a stamp exists, mine must differ.)
...
25. Does the accent colour cover more than the budget for the active genre? Default budget is **~5 % of any single viewport** (count by area: solid fills, large headings in accent, full-bleed accent backgrounds); anything **above 10 % is a hard fail** in every genre except atmospheric. If over budget, retreat — accent is for emphasis, not for filling. *Genre note: atmospheric allows accent-tinted radial blooms covering ~20–30 % of the canvas, background-only, since the bloom is the design.*
26. Is any padding / gap / margin a value that isn't on the named spacing scale (`--space-3xs` 2px … `--space-4xl` 144px, per layout.md)? Arbitrary `padding: 17px` is a tell.
...
28. Does any interactive element lack `:focus-visible`, `:active`, OR `:disabled` styling? (Nine states is the canon — rest / hover / focus-visible / active / disabled / loading / selected / empty / error, per [`interaction-states.md`](../rules/interaction-states.md). In code, at minimum rest + hover + focus-visible + active + disabled must be present on every interactive element; loading / selected / empty / error whenever the element can enter them.)
...
(When the page carries enrichment — see [`hero-enrichment.md`](../rules/hero-enrichment.md).)
...
(Three faces is the ceiling. See [`typography.md` § The 2+1 rule](../rules/typography.md).)
...
(Inputs are where almost-right UIs lose. See [`interaction-states.md` § Input field states](../rules/interaction-states.md).)
...
49. When `--color-accent` is used as a fill anywhere on the page (button, badge, surface), is `--color-accent-ink` **also defined** (in `:root` or theme tokens) AND used as the `color` for text on that fill? If `--color-accent-ink` is missing, Studio output is one careless `color: white` away from a low-contrast accident. The token must exist, must verify ≥ APCA Lc 60 / WCAG 4.5:1 against `--color-accent`, and must be applied wherever accent fills a surface that carries text.
...
The CSS stamp at STEP 9 should record the result: `· contrast: pass (46–50)` if all five gates pass, or `· contrast: FAIL gates <list>` if any are open. Fix before shipping.
...
The CSS stamp at STEP 9 should record the result alongside contrast: `· nav: N# · footer: Ft# · slop: pass (51–55)`. If any of 51–55 fail, fix before shipping.
...
56. **Invented metric.** Does the page contain any quantitative claim — "10× faster", "saves 5 hours per week", "trusted by 50,000+ teams", "99.9 % uptime", "+47 % conversion" — that the user did not supply, that has no source, and that the model fabricated to fill a stat-led layout, comparison row, or proof bar? If yes, fail. The fix is one of: replace the number with `—` and a labelled grey block, replace it with a question to the user ("metric to confirm"), or rebuild the section without the proof slot. Stat-led macrostructures are slop the moment their stats become decorative. *(See [anti-patterns.md § Invented metrics](../rules/anti-patterns.md).)*

## Re-drawn UI chrome

Universal. Studio must reuse the user's existing chrome (browser, OS, IDE) instead of redrawing it.

57. **Re-drawn chrome.** Did Studio hand-build a fake browser bar (URL pill + traffic-light dots), a fake phone frame (rounded rectangle + notch + speaker slit), a fake code-block frame (mock window-chrome around a `<pre>`), a fake terminal frame, or a fake IDE chrome (file tabs + activity bar + sidebar) using HTML/CSS or SVG? If yes, fail. Re-drawn chrome is one of the strongest "looks AI-generated" tells — the model invented a UI that already exists in the user's environment. The fix: use a `<picture>` or `<figure>` containing a real screenshot, or omit the chrome and let the content stand on its own. *(See [anti-patterns.md § Re-drawn UI chrome](../rules/anti-patterns.md).)*
...
58. **Mid-render token improvisation.** Did Studio introduce any colour value (`#hex`, `oklch(...)`, `rgb(...)`, `hsl(...)`) or `font-family` declaration *outside* the design tokens defined in `:root` / `[data-theme="..."]`? If yes, fail. Every colour and every font in the artifact must reference a named token (`var(--color-accent)`, `font-family: var(--font-display)`). Inline OKLCH or one-off hexes are mid-render improvisation — the model picked the theme, then forgot it and freestyled. The fix: lift the value into the token block as a new named variable, or replace it with an existing token. *(See [SKILL.md § DESIGN STORE](../../SKILL.md) and [anti-patterns.md § Mid-render token improvisation](../rules/anti-patterns.md).)*
...
69. **Studied DNA discarded for a catalog theme.** Did a `study` diagnosis emit earlier in the conversation, AND does the build's CSS stamp's `theme:` field name a catalog theme (Specimen, Atelier, Brutal, Salon, Newsprint, Linen, Studio, Manifesto, Terminal, Midnight, Almanac, Garden, Quiet, Riso, Sport, Bloom, Coral, Violet, Aurora, Halo, Plume, Editorial) rather than `studied-DNA (source: ...)` — without the user having explicitly pivoted ("use Linen instead", "ignore the DNA", "rotate to a different theme")? Auto-fail. The studied DNA was meant to be the system (see [`study.md`](study.md) — studied DNA overrides the catalog); defaulting back to catalog is the attractor pull. Fix: re-emit using the studied DNA's tokens directly (paper OKLCH, accent OKLCH, named candidate fonts, macrostructure, archetypes) and update the stamp to `theme: studied-DNA (source: <URL or image>)` with the inline values. This gate is trivially passed when no recent study exists in conversation scope.

The CSS stamp at STEP 9 records mobile pass alongside contrast: `· mobile: pass (36, 59, 61–69)`.

## Thesis coherence

Universal on full designs. The thesis (`state.thesis_position` / `state.thesis_refusals`, derived at STEP 1 per [`../rules/thesis.md`](../rules/thesis.md)) is the spine; these gates catch a design with gates-green craft but no point of view, and a thesis that leaked into copy as self-importance.

70. **Catalog-default decisions un-moored from the thesis.** Walk the 8 major decisions — genre, macrostructure, theme, color, type, skeleton, motion, copy. For each, can you finish "I chose X *because the thesis calls for* ___" with something other than "it was the catalog default / the router routed there"? If more than 2 of the 8 are catalog-default rather than thesis-driven, fail. This is the gates-green-but-soulless failure: every threshold passes, yet nothing argues anything. Fix: re-run each un-moored STEP deriving the pick from `state.thesis_position` (warm accent because the thesis argues openness; one dominant action because it refuses distraction; etc.). Record the corrected picks. A design may pass every other gate and still fail here — that is the point.

71. **Thesis leaked into shipped copy as a manifesto.** Does any user-facing string (hero, onboarding, brand line, empty state, about blurb) state the *philosophy* of the design rather than an observable user benefit? Auto-fail. Tells: "we refuse the tyranny of…", "we believe in the poetry of…", "transparency-first", "we embrace constraint", or any sentence a user wouldn't care about because it's written for designers to admire. The thesis is a *design constraint*, not page copy (thesis.md T5). Fix: rewrite as the benefit the user can observe — "We refuse the infinite list" → "You always see your five real priorities, nothing else." The position should be *felt through the design*, never *announced in the copy*.

The CSS stamp at STEP 9 records: `· thesis: pass (70, 71)`.
