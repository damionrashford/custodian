...
Forty-six component archetypes you can compose into any macrostructure. Every entry: a *shape*, a one-line "use when", a one-line "don't confuse with", and a short structural sketch (DOM + minimal CSS). Pick from this file when you're building a section and don't know which shape to reach for.
...
**Diversification rule:** within a single page, no two sections should use the same archetype. A Bento Grid might pair *Bento feature block* with *Inline form CTA* with *Logo wall (hairline)*. The next page Studio builds should pick different archetypes from the same categories.

---

## Archetype index — load ONLY the picks you need

**Pick your archetype names here, then read ONLY those individual files** from `components/`. Do not load the whole cookbook. A typical build needs 5–7 files: 1 hero + 1 section head + 1–2 features + 1 CTA + 1 footer + 1 nav.
...
- **Ft5 · Statement** — One large display sentence dominates the footer — a closing line, not a sitemap. Wordmark, minimal links, copyright sit beneath in muted small type. Stripe (older), Mailchimp pre-rebrand, agency portfolio closers. [`components/ft5-statement.md`](components/ft5-statement.md)
...
- **N6 · Newspaper masthead** — Full-width header, large centred wordmark on the top row, thin issue/date line above or below in serif small caps, optional inline link row beneath, double-rule below the whole thing. Editorial broadsheet energy — NYT, FT, Vogue. [`components/n6-newspaper-masthead.md`](components/n6-newspaper-masthead.md)
- **N7 · Brutal slab** — A heavy, full-width nav with a 2 px solid border-bottom, all-caps wordmark and tracked uppercase link row, dense rhythm, no shadow, no rounded corners. Reads as Pentagram project pages, brutalist-leaning agencies. [`components/n7-brutal-slab.md`](components/n7-brutal-slab.md)
- **N8 · Terminal command** — A nav formatted as a CLI prompt: `> studio --catalog --voice --get▮`. The "links" are command flags. The blinking cursor (`▮`) is allowed *only here* (it has purpose — signals "you'd type next"); never standalone elsewhere. [`components/n8-terminal-command.md`](components/n8-terminal-command.md)
...
- **N10 · Floating on scroll morph** — A sticky bar at the top that **morphs into a floating pill** as the user scrolls past a threshold. Two visual modes share one DOM — `.nav` (outer) owns the bar look, `.nav__inner` (inner) owns the pill look, cross-faded on a single class toggle. [`components/n10-floating-on-scroll-morph.md`](components/n10-floating-on-scroll-morph.md)
...
```css
/* Studio · macrostructure: Bento Grid · F1 Bento knobs: tiles=6, spans=irregular, accent=corner-only · ... */
```
...
| **S1 Left-margin numbered** | Number style: `01 —` · `No. 1` · roman `I.` | Label case: small caps · uppercase mono · italic | Column width: 10ch · 14ch · 18ch |
| **S2 Hanging** | Offset above: half-space · full-space · 1.5× | Size: xl · 2xl · display | Alignment: left · centred (≤6 words only) |
| **S3 Sticky pinned** | Pin side: left · top | Unpin at: section end · next heading | Weight: display · body-large caps |
| **S4 Inline no-break** | Case: small caps · uppercase tracked | Separator before: none · middot · rule fragment | Colour: ink · muted · accent (sparingly) |
| **S5 Bottom anchored** | Anchor treatment: rule above · negative space only | Size: sm caps · body | Alignment: left · right-flush |
...
| **N10 Floating-on-scroll morph** | Threshold: 64 px · after hero · 25 % scroll | Pill width: content · ~560 px · ~720 px | Morph timing: 200 ms ease-out · 280 ms spring-soft |
...
Per-theme defaults are canonical in [`themes.md`](../rules/themes.md) / `structure.md`; these clusters are the convenience view:

| Theme cluster | Default | Also OK |
| --- | --- | --- |
| mast-headed print (Editorial · Salon · Linen) + soft-minimal (Coral · Plume) | **Ft1 Mast-headed** | Ft2, Ft6, Ft7 |
| inline-minimal (Specimen · Quiet · Violet · Midnight) | **Ft2 Inline single line** | Ft1, Ft5 |
| statement close (Manifesto · Bloom · Aurora · Halo) | **Ft5 Statement** | Ft1, Ft2 |
| marquee-loud (Brutal · Sport · Riso) | **Ft8 Marquee scroll** | Ft5, Ft7 |
| colophon (Newsprint · Terminal) | **Ft4 Dense colophon** | Ft2 |
| index/hub (Almanac · Studio) | **Ft3 Index columns** | Ft1 |
| letter close (Atelier · Garden) | **Ft6 Letter close** | Ft2 |

**Diversification.** Same rule as nav — differ from the last 3 runs in `.studio/log.json`; no two consecutive outputs share the same footer archetype.
...
**Theme→genre membership is canonical in [`themes.md`](../rules/themes.md)** — the clusters in parentheses below are convenience copies; when they disagree, themes.md wins.

| Theme cluster | Default nav | Acceptable also |
| --- | --- | --- |
| masthead print (Editorial · Salon · Newsprint · Linen) | **N6 Masthead** | N1, N9 |
| floating minimal (Specimen · Coral · Violet) | **N5 Floating pill** | N1, N9, N10 |
| atmospheric dark (Bloom · Aurora · Halo · Midnight) | **N5 Floating pill** (blur backdrop sells the mood) | N9, N4, N10 |
| edge-quiet (Quiet · Atelier · Garden · Plume) | **N9 Edge-aligned minimal** | N1, N5 |
| loud slab (Brutal · Manifesto · Sport · Riso · Studio) | **N7 Brutal slab** | N1, N3 |
| terminal / CLI (Terminal) | **N8 Terminal command** | N4 ⌘K-only |
| docs / reference (Almanac) | **N3 Side-rail** | N1, N4 |

**Diversification.** Differ from the last 3 runs in `.studio/log.json` — no two of them share a nav archetype, even when they share a genre. If the previous run used N5 on a modern-minimal page, the next modern-minimal page picks N1 or N9 from the routing table's "also" column.
...
The goal is composed variety — within a page, sections feel different from each other; across pages Studio builds, sections feel different from the last.
...
| **S1 Left-margin numbered** | label column collapses; number + label stack ABOVE the heading in one column (gate 66 shape) | number size step down; label tracks tighter |
| **S2 Hanging** | unchanged (single column already) | offset above reduces one space step |
| **S3 Sticky pinned** | unsticks; renders as a static heading above content | unchanged from 60 rem |
| **S4 Inline no-break** | unchanged | unchanged |
| **S5 Bottom anchored** | unchanged | rule above tightens; size step down |
...
| **N10 Floating-on-scroll morph** | morph disabled; renders as its pill state permanently (scroll-linked morphs fight mobile scroll physics) | pill collapses to corner chip — wordmark + hamburger |
