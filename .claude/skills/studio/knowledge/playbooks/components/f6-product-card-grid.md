...
```
```css
.product-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: var(--space-xl) var(--space-lg); } /* minmax(0,1fr): image tracks must never inherit intrinsic min-width (gate 61) */
