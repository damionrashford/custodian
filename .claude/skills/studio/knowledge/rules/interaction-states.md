...
Every interactive element has **nine states** — the studio canon, matched by SKILL.md STEP 6, slop gate 28, and critique check 14. Most AI-generated UI styles two (rest, hover) and forgets the rest. That's where interfaces break.

## The nine states

| State | When | Treatment |
| --- | --- | --- |
| Rest | At rest | Base styling |
| Hover | Pointer over (only with `@media (hover: hover)`) | Small shift: colour, 1px translate, subtle border |
| Focus-visible | Keyboard or programmatic focus | Visible ring, `:focus-visible` |
| Active / Pressed | During press | Pressed-in: darker, translate(0 1px), or inset shadow |
...
| Selected | Chosen among peers (tab, list row, toggle, chip) | Distinct fill or rule; NEVER colour alone — pair with weight, icon, or border |
| Empty | The element/collection has no content yet | Explanation + action (see § Loading and empty states); never a bare "No results" |
| Error | Failed state | Error-colour border, icon, message, `aria-invalid` |

**Success** is a *feedback moment*, not a persistent state: show it via quiet confirmation (subtle border + ✓ that auto-clears — see the input recipe below), and prefer silent success when the effect is already visible (gate 17).

In code, every interactive element must ship at minimum rest + hover + focus-visible + active + disabled; loading / selected / empty / error whenever the element can enter them. If a reachable state is missing on a production element, the element isn't finished.
...
Studio output must pass slop-test gates 46–50 before shipping. Compute contrast for every `(color, background-color)` pair on the page. The common failures Studio output trips on:
...
Every theme MUST define `--color-accent-ink` — the text colour to use whenever `--color-accent` fills a surface that carries text. The accent-ink colour is verified ≥ APCA Lc 60 against the accent at the time the theme is built. Studio code that uses `background: var(--color-accent)` must also set `color: var(--color-accent-ink)`. Falling back to hardcoded `color: white` is a tell — the theme's accent could be a light colour, and white-on-light is the bug.
